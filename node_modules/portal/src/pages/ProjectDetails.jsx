...
export default function ProjectDetails() {
    ...
    const handleSaveBudget = async (item) => {
        ...
        // Jika id mengandung 'unbudgeted-', berarti ini item baru yang asalnya dari transaksi
        if (String(item.id).startsWith('unbudgeted-')) {
            updatedList = [...budgetItems, {
                id: Date.now(),
            ...
            // KONDISI EDIT BIASA ATAU BARU SEPENUHNYA
            let exists = budgetItems.find(i => String(i.id) === String(item.id));
            if (exists) {
                // KONDISI EDIT: Update data yang sudah ada
                updatedList = budgetItems.map(i => String(i.id) === String(item.id) ? { ...i, ...item } : i);
            } else {
    ...
    // ----- HYBRID BUDGET & TRANSACTION AGGREGATOR -----
    const computedBudgetItems = React.useMemo(() => {
        // 1. Aggregate transaction sums per material name
        const expenseAggregates = {};

        projectTransactions.forEach(trx => {
            if (trx.type === 'out') {
                const materialName = trx.title || 'Unknown Item';
                if (!expenseAggregates[materialName]) {
                    expenseAggregates[materialName] = {
                        qtyUsed: 0,
                        totalUsedNum: 0,
                        category: trx.category || 'LAIN-LAIN'
                    };
                }
                expenseAggregates[materialName].qtyUsed += Number(trx.qty || 0);
                expenseAggregates[materialName].totalUsedNum += Number(trx.amount || 0);
            }
        });

        // 2. Map existing budgets and apply aggregated usages
        const mergedItems = budgetItems.map(item => {
            const agg = expenseAggregates[item.name];
            const parsedBudget = parseCurrency(item.totalBudget);
            let qtyUsed = 0;
            let totalUsedNum = 0;

            if (agg) {
                qtyUsed = agg.qtyUsed;
                totalUsedNum = agg.totalUsedNum;
                // Mark this aggregate as processed so we don't duplicate it later
                agg.processed = true;
            }

            const parsedQtyTotal = Number(item.qtyTotal) || 1; // Prevent division by zero
            const qtyPercentage = Math.min((qtyUsed / parsedQtyTotal) * 100, 100).toFixed(0);

            // Re-calculate effective properties based on main project progress
            const effectiveBudgetNum = (parsedBudget * progress) / 100;
            const effectivePercentage = effectiveBudgetNum > 0 ? Math.min((totalUsedNum / effectiveBudgetNum) * 100, 100).toFixed(0) : 0;

            const isOverBudget = totalUsedNum > effectiveBudgetNum;

            return {
                ...item,
                qtyUsed: qtyUsed,
                qtyPercentage: qtyPercentage,
                effectiveBudget: formatCurrency(effectiveBudgetNum),
                totalUsed: formatCurrency(totalUsedNum),
                effectivePercentage: effectivePercentage,
                status: isOverBudget ? 'Over Budget' : 'Safe',
                statusColor: isOverBudget ? 'red' : 'green'
            };
        });

        // 3. Inject synthetic budgets for items that have transactions but NO formal budget
        Object.entries(expenseAggregates).forEach(([matName, agg]) => {
            if (!agg.processed) {
                mergedItems.push({
                    id: `unbudgeted-${matName}`, // Prefix ensures distinct tracing
                    category: agg.category,
                    subCategory: '-',
                    name: matName,
                    spec: 'Unbudgeted Output',
                    qtyUsed: agg.qtyUsed,
                    qtyTotal: 0,
                    qtyUnit: 'Unit',
                    qtyPercentage: 100,
                    totalBudget: formatCurrency(0),
                    effectiveBudget: formatCurrency(0),
                    totalUsed: formatCurrency(agg.totalUsedNum),
        ...
        // 4. Sort the final merged list alphabetically by category A-Z
                                ...
                                <div className={`overflow-x-auto overflow-y-scroll custom-scrollbar transition-all duration-600 ease-linear ${isBudgetExpanded ? 'max-h-[2000px]' : 'max-h-[500px]'}`}>
                                    <table className="w-full text-sm text-left table-fixed">
                                        ...
                                        <tbody className="divide-y divide-gray-200 dark:divide-border-dark">
                                            {computedBudgetItems.map((item, index) => {
                                                const showHeader = index === 0 || item.category !== computedBudgetItems[index - 1].category;
                                                            ...
                                                            <td className="px-6 py-4 text-right align-top">
                                                                <div className="flex justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                                    <button
                                                                        ...
                                                                        title={String(item.id).startsWith('unbudgeted-') ? "Buat Anggaran Baru dari Transaksi" : "Edit"}
                                                                    >
                                                                        <span className="material-icons-round text-[18px]">
                                                                            {String(item.id).startsWith('unbudgeted-') ? 'add_task' : 'edit'}
                                                                        </span>
                                                                    </button>
                                                                    {!String(item.id).startsWith('unbudgeted-') && (
                                                                        <button
